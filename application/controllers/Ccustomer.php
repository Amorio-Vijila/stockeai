...
class Ccustomer extends CI_Controller {
 ...
 public function add_customertype_new() {
    $this->form_validation->set_rules('new_customer_type', 'Customer Type', 'required|callback_alpha_space');
    ...
    public function getCustomerDatas() {
        $encodedId      = isset($_GET['id']) ? $_GET['id'] : null;
        $encoded_admin      = isset($_GET['admin']) ? $_GET['admin'] : null;
        $decodedId      = decodeBase64UrlParameter($encodedId);
        $decoded_admin      = decodeBase64UrlParameter($encoded_admin);
        ...
        foreach ($items as $item) {
            $edit   = '<a href="' . base_url('Ccustomer/customer_update_form?id=' . $encodedId. '&admin=' . $encoded_admin  . '&customer_id=' . $item['customer_id']) . '" class="btnclr btn btn-sm" style="background-color:#424f5c; margin-right: 5px;"><i class="fa fa-pencil" aria-hidden="true"></i></a>';
            $delete = '<a href="' . base_url('Ccustomer/customer_delete?id=' . $encodedId. '&admin=' . $encoded_admin  . '&customer_id=' . $item['customer_id']) . '" class="btn btn-sm btn-danger" ><i class="fa fa-trash" aria-hidden="true"></i></a>';
            $row = [
                "customer_id"     => $i,
...
$this->form_validation->set_message('alpha_space', 'The {field} field should only contain alphabets and spaces.');
    ...
    public function customer_delete() {
        ...
        $this->session->set_userdata(array('message' => display('successfully_delete')));
            redirect(base_url('Ccustomer/manage_customer?id=' . $_GET['id'].'&admin='. $_GET['admin']));
        }
    }
